...
import 'package:e_ktg_mobile/presentation/fasilitas/detail/comment/comment_resto_page.dart';
...
class FasilitasRestoDetailPage extends StatefulWidget {
  final Resto data;

  const FasilitasRestoDetailPage({super.key, required this.data});

  @override
  State<FasilitasRestoDetailPage> createState() =>
      _FasilitasRestoDetailPageState();
}

class _FasilitasRestoDetailPageState extends State<FasilitasRestoDetailPage> {
  ...
  Future<void> _refreshResto() async {
    context.read<RestoBloc>().add(RestoEvent.getAllResto());
  }
      ...
      child: BlocBuilder<RestoBloc, RestoState>(
        builder: (context, state) {
          return state.maybeWhen(
            ...
            success: (restoList) {
              final updatedData = restoList.data.firstWhere(
  ...
  Widget _buildContent(BuildContext context, Resto data) {
  ...
  Widget _buildInfoSection(Resto data) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(
